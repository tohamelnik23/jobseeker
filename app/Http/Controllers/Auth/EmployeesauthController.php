...
class EmployeesauthController extends Controller{ 
	...
	public function register(Request $request){
		return view('auth/registeremployees');
    }
	/**
    ...
    protected function create(Request $request){  
		$rules = array(
			'firstname'              => 'required',
			'lastname'               => 'required',
			'cphonenumber'           => 'required | unique:users', 
		...
		$message= array(
			...
			'cphonenumber.unique'=>'Please provide a valid phone number.',
			'cphonenumber.required'=>'Please provide a valid phone number.', 
        ...
        if( $validator->fails() ):  
			...
			//SlackInfo::postMessageForError("notifications", "The employee " . $user_account->name  . " has been registered"); 
			return redirect()->route('login')->withInput()->with('success','An email has been sent to your email with your new account details.  Please follow the directions to complete your registration.');
		endif;
    }
}