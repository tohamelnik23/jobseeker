    ...
    <tr>
        @foreach($timesheets as $timesheet)
            ...
            <td  class = "text-center vertical-align-middle">
                @if( ($now >= $timesheet['real_date'])    &&  ( date('Y-m-d', strtotime($offer->start_time)) <= $timesheet['real_date'] ) )
                    @if(isset($timesheet['time_sheet']))
                        @if($enable_edit)
                            <input type = "text" value = "{!! date('H:i', strtotime($timesheet['time_sheet']->timesheets_time))  !!}" class = "form-control  text-center selected_time_input"  data-date = "{!! $timesheet['real_date'] !!}"  />
                        @else
                            <p class = "mar-no">
                    ...
                    @else
                        @if($enable_edit)
                            <input type = "text" value = "00:00" class = "form-control text-center selected_time_input"  data-date = "{!! $timesheet['real_date'] !!}"  />
                        @else
                            <p class = "mar-no"> - </p>
                        @endif
                    @endif 
        ...
        @endforeach            
            ...
            <td  class = "text-center vertical-align-middle hourly_rate_amount" data-amount = "{!! $offer->amount !!}">
                <p class = "mar-no">  ${!! number_format($offer->amount, 2) !!}/hr </p>
            </td>
