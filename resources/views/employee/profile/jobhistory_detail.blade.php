...
<form id="updatejobhistoryForm" method="post" class="form-horizontal" action="{{{ route('employee.update.jobhistory') }}}">
    @csrf
    <input type="hidden" name="jobhistory" value="{{$jobhistory->serial}}"> 
    <div class="row">
		<div class="form-group col-lg-12">
			...
			<div class="col-sm-12  @if ($errors->has('job_title')) has-error @endif">
				<input type = "text" class="form-control" name="job_title" placeholder="Ex: Main Developer" value="{{old('job_title',$jobhistory->job_title)}}"  maxlength = "512" />
    ...
    <div class="row">
		<div class="form-group col-lg-12">
			...
			<div class="col-sm-12  @if ($errors->has('job_company')) has-error @endif">
				<input type = "text" class="form-control"   name="job_company" placeholder="Ex: Microsoft" value="{{old('job_title', $jobhistory->job_company)}}" maxlength = "512"/>
    ...
    <div class="row">
            ...
            <div class="col-sm-3">
                <select class = "form-control" name = "job_start_date_year">
                    @for($i = 1980 ; $i <= date('Y'); $i++)
                        <option value = "{!! $i !!}"  @if($jobhistory->job_start_date_year == $i) selected @endif >{!! $i !!}</option>
                    @endfor
                </select>
            </div>  
            <div class="col-sm-3">
                <select class = "form-control" name = "job_start_date_month">
                    ...
                    @foreach($months_array as $month_key => $month)
                        <option value = "{!! $month_key + 1 !!}" @if($jobhistory->job_start_date_month ==  ( $month_key + 1 ) ) selected @endif  >{!! $month !!}</option>
                    @endforeach
                </select>
            </div> 
        </div>
        ...
        <div class="form-group col-lg-12">
            ...
            <div class="col-sm-3">
                <select class = "form-control" name = "job_end_date_year">
                    @for($i = 1980 ; $i <= date('Y'); $i++)
                        <option value = "{!! $i !!}"  @if($jobhistory->job_end_date_year == $i) selected @endif >{!! $i !!}</option>
                    @endfor
                </select>
            </div>  
            <div class="col-sm-3">
                <select class = "form-control" name = "job_end_date_month">
                    ...
                    @endphp
                    @foreach($months_array as $month_key => $month)
                        <option value = "{!! $month_key + 1 !!}" @if($jobhistory->job_end_date_month ==  ( $month_key + 1 ) ) selected @endif >{!! $month !!}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="form-group col-lg-12">
            ...
            <div class="col-sm-12  @if ($errors->has('job_description')) has-error @endif">
				<textarea rows = "5"  maxlength = "65535" type = "text" class="form-control" id="job_description" name="job_description">{{old('job_description', $jobhistory->job_description)}}</textarea>
    ...
    <div class="row my-lg-3">
		<div class="form-group col-lg-12">
			<div class="col-sm-12">
				<button  id="editjobhistorysubmit" type="submit" class="btn btn-primary" name="save" value="Save Changes">Save Changes</button>
